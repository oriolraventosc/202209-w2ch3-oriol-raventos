...
function admin() {
  if (vuelos.length < 16) {
    crearVuelos = prompt(
      "Quieres crear un nuevo vuelo o cerrar la sesión? si / no / end"
    );
    if (crearVuelos == null) {
      console.log("Introduce si / no");
      return admin();
    ...
    if (crearVuelos.trim().length > 0) {
      ...
      if (crearVuelos.toLowerCase() === "no") {
        vuelos.forEach((vuelo) => {
          console.log(
            `${vuelo.id} | ${vuelo.from} - ${vuelo.to} | ${vuelo.cost}€`
          );
        });
        eliminarVuelo();
...
function eliminarVuelo() {
  eliminarVuelos = prompt(
    "Quieres eliminar algun vuelo o cerrar la sesión? si / no / end"
  );
...
function buscarID() {
  let idVueloEliminar = prompt(
    "Escribe el número ID del vuelo que quieres eliminar. Ejemplo: 9"
  );
...
function user() {
  preguntaPrecio = prompt(
    "Quieres buscar vuelos por precio o cerrar la sesión? si / no / end"
  );
...
const m = MyMin(vuelos);

function buscarPrecioF() {
  vuelosFilterPrecio = [];
  buscarPrecio = prompt(
    "Cuanto te quieres gastar en tu vuelo? *Escribe el precio* Ejemplo: 490"
  );
  vuelos.forEach((vuelo) => {
    ...
    if (buscarPrecio.trim().length > 0) {
      if (vuelo.cost <= buscarPrecio) {
        vuelosFilterPrecio.push(
          `El vuelo con destino ${vuelo.to} tiene un precio de ${vuelo.cost}.`
        );
...
function containsNumberPrecio() {
  return /[0-9]/.test(precio);
}

function containsLetterOrigen() {
  return /[a-zA-Z]/.test(origen);
}

function containsLetterDestino() {
